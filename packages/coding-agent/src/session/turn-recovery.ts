...
export class TurnRecovery {
	...
	persistTerminalEmptyErrorTurn(message: AssistantMessage, force = false): Promise<void> {
		return this.#persistTerminalEmptyErrorTurn(message, force);
	}
	 ...
	 * the already-persisted guard unless `force` is requested for a distinct
	 * terminal error after an exhausted retry. The turn is dropped from active
	 * context by the caller (or `isProviderRefusalMessage`/`isEmptyErrorTurn`
	 * filters) so it is never replayed on the wire. Used by the retry-lifecycle
	 * dead-ends and the non-retry terminal error tail.
	 */
	async #persistTerminalEmptyErrorTurn(message: AssistantMessage, force = false): Promise<void> {
		await this.#host.waitForSessionMessagePersistence(message);
		if (!isEmptyErrorTurn(message)) return;
		if (!force && this.#host.sessionMessageAlreadyPersisted(message)) return;
		this.#host.appendSessionMessage(structuredClone(message));
	}
	...
	async #handleRetryableError(
		...
		if (retryBudgetExhausted) {
			if (!switchedModel && !switchedCredential) {
				...
				await this.persistTerminalEmptyErrorTurn(message, true);
