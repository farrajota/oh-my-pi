...
export class SessionFocusController {
	...
	async #detachToMain(): Promise<void> {
		...
		if (attached && !this.#focusedAgentId) this.ctx.showStatus("Returned to main session");
	}
		...
		try {
			...
			const restoreAssistant = async (message: AssistantMessage): Promise<void> => {
				...
				if (!assistantStreamSynced) {
					...
					await this.ctx.eventController.handleEvent(target, { type: "message_start", message });
					if (generation !== this.#attachGeneration) return;
				}
				await this.ctx.eventController.handleEvent(target, {
					type: "message_update",
					message,
