...
import { runProviderSetupWizard } from "./setup";
import { interruptHint, sanitizeStatusText } from "@oh-my-pi/pi-tui/chrome/shared";
...
import { type ShimmerPalette, shimmerEnabled, shimmerSegments, shimmerText } from "@oh-my-pi/pi-tui/theme/shimmer";
...
/** Intern shimmer palettes per accent to keep loader redraws cache-friendly. */
...
function renderWorkingMessage(message: string, accent?: WorkingMessageAccent, suffix = ""): string {
	...
	if (accent) accent.palette ??= palette;
...
export class InteractiveMode implements InteractiveModeContext {
	...
	tokenRate = new TokenRateMeter(text => {
	...
	/** Generation tok/s for the working row. */
	...
	constructor(
		...
		this.statusLine.setVibeWorkerTokenRateProvider(() =>
			aggregateVibeWorkerTokensPerSecond(
				this.collabGuest?.agentRegistry ?? AgentRegistry.global(),
	...
	async init(options: InteractiveModeInitOptions = {}): Promise<void> {
		...
		this.#streamPublisher =
			(await StreamPublisher.connectLazy({
				...
				tui: this.ui,
				loadRedactor: () => StreamRedactor.load(streamCwd, []),
				onStatus: status => {
	...
	startPendingSubmission(
		...
		this.#pendingSubmissionPreservesDraft = options?.preserveDraft ?? false;
	...
	syncRunningSubagentBadge(options: { requestRender?: boolean } = {}): void {
		const registry = getRunningSubagentBadgeRegistry(
			this.collabGuest ? { agentRegistry: toAgentHubRegistry(this.collabGuest.agentRegistry) } : undefined,
			toAgentHubRegistry(AgentRegistry.global()),
		);
	...
	#vibeParentSession(): VibeParentSession {
		return {
			...
			agentRegistry: this.collabGuest?.agentRegistry ?? AgentRegistry.global(),
		};
	}
