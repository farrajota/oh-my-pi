...
export class InspectImageTool implements AgentTool<typeof inspectImageSchema, InspectImageToolDetails> {
	...
	async execute(
		...
		const formatTimeoutMessage = (): string => {
			...
			return `inspect_image request timed out after ${seconds}s. Increase images.questionTimeoutMs (currently ${timeoutMs}ms; 0 disables) or check the vision model provider.`;
		};
