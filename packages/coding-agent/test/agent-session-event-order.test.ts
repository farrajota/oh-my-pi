...
describe("AgentSession subscriber event order", () => {
	...
	it("publishes the captured agent-start time only when subscriber delivery begins", async () => {
		const model = getBundledModel("anthropic", "claude-sonnet-4-5");
		if (!model) throw new Error("Expected bundled test model to exist");
		...
		session.subscribe(event => {
			if (event.type === "agent_start") observedStartTimes.push(session?.activeRunStartedAt);
		});
