...
function makeSession(cwd: string, artifacts: ArtifactManager): ToolSession {
	return {
		cwd,
		hasUI: false,
...
describe("read tool large artifact handling", () => {
	...
	it("blocks unbounded raw reads and points to bounded artifact workflows", async () => {
		const result = await tool.execute("call-raw", { path: `artifact://${artifactId}:raw` });
	...
	it("returns exactly the requested raw artifact range without context padding", async () => {
		const result = await tool.execute("call-raw-exact", { path: `artifact://${artifactId}:raw:31-31` });
		const output = getTextOutput(result);
	...
	it("records the source line count for an open-ended artifact range that reaches EOF", async () => {
		const result = await tool.execute("call-raw-tail", { path: `artifact://${artifactId}:raw:301-` });
	...
	it("keeps the continuation when the byte budget stops inside requested artifact content", async () => {
		const id = await artifactManager.save(byteLimitedRangeArtifact(), "mcp");

		const result = await tool.execute("call-byte-limited-range", { path: `artifact://${id}:1-100` });
		const output = getTextOutput(result);
	...
	it("reports an oversized selected line instead of sending a looping continuation selector", async () => {
		const id = await artifactManager.save(oversizedSelectedLineArtifact(), "mcp");

		const result = await tool.execute("call-oversized-selected", { path: `artifact://${id}:2-2` });
		const output = getTextOutput(result);
		...
		expect(output).toContain(`artifact://${id}:raw:2-2`);
		const truncation = result.details?.meta?.truncation;
	...
	it("still returns the oversized selected line when a wider range raises the byte budget", async () => {
		...
		const result = await tool.execute("call-wide-oversized-selected", { path: `artifact://${id}:2-142` });
		const output = getTextOutput(result);
	...
	it("keeps raw oversized-line reads context-free and byte-capped", async () => {
		...
		const result = await tool.execute("call-raw-oversized-selected", { path: `artifact://${id}:raw:2-2` });
		const output = getTextOutput(result);
	...
	it("tails an artifact with :-N by counting lines first, then streaming only that window", async () => {
		const output = getTextOutput(await tool.execute("call-tail", { path: `artifact://${artifactId}:-3` }));
		...
		const raw = getTextOutput(await tool.execute("call-raw-tail-n", { path: `artifact://${artifactId}:raw:-2` }));
	...
	it("shortens artifact paths under the user's home dir instead of leaking the absolute path", async () => {
		...
		try {
			const result = await tool.execute("call-raw-home", { path: `artifact://${artifactId}:raw` });
			const output = getTextOutput(result);
