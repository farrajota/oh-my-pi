...
function taskPermissionProfiles(value: unknown): string {
	if (!value || typeof value !== "object" || !("profiles" in value) || !Array.isArray(value.profiles)) return "";
	const profiles = value.profiles.filter((profile): profile is string => typeof profile === "string");
	return profiles.length > 0 ? `Profiles: ${profiles.join(", ")}` : "";
}
...
export function renderCall(args: TaskParams, options: TaskRenderOptions, theme: Theme): Component {
	...
	const headerMeta = [args.model, showIsolated ? "isolated" : undefined].filter(Boolean).join(" · ") || undefined;
