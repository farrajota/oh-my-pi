...
async function openInitialCodexEventStream(
	...
	try {
		return await openCodexSseTransport(
			model,
			requestContext,
			requestSetup,
			options,
...
export async function openCodexCompactionEventStream(
	...
	try {
		...
		initial = await openInitialCodexEventStream(model, options, requestSetup, requestContext, transportAttemptState);
	} catch (error) {
	...
	return streamCodexCompactionEvents(model, options, requestSetup, requestContext, transportAttemptState, initial);
}
	...
	try {
		if (initial.transport === "websocket") {
			...
			try {
				...
				const fallback = await openCodexSseTransport(
					model,
					requestContext,
					requestSetup,
					options,
			...
			if (
				...
				const replay = await openCodexSseTransport(
					model,
					requestContext,
					requestSetup,
					options,
...
async function openCodexSseEventStream(
	...
	const fetchAttempt: FetchImpl = async (input, init) => {
		...
		try {
			response = await (fetchOverride ?? globalThis.fetch)(input, init);
			return response;
		} finally {
